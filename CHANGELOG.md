...
- Pinned `hai-agent-runtime` 0.1.12: self-hosted endpoints work without `HAI_API_KEY`, a wrong base URL fails immediately, desktop actions wait 1 s before the next one, screenshots are sent in full on every request (no `image_cache_miss` round-trip on the hosted API, multi-step runs work on llama.cpp), and `win`/`escape`/`return`/`pageup` key names work in hotkeys.
- `holo run` names the fix (`holo login --force`) when the hosted gateway rejects the API key.
- `holo stop --force` only kills pids that still belong to a `hai-agent-runtime` process.
- `holo mcp` reports the CLI version as its server version.
